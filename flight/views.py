...
import math
from .models import *
from flynext.utils import render_to_pdf, createticket
...
def faq(request):
    ...
    return render(request, 'flight/faq.html', {'success': success})
...
def contact(request):
    ...
    # Check if the 'success' parameter is in the query string
    success = request.GET.get('success')
    return render(request, 'flight/contact.html', {'success': success})

    
def error(request):
    return render(request, 'flight/404.html')
...
def profile(request):
    ...
    if request.method == 'POST' and not is_google_user:
    ...
    return render(request, 'flight/profile.html', context)
